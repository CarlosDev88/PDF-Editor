...
signer_service = PdfSignerService()
...
def api_signer_load_signature() -> dict:
    ...
    try:
        ...
        # Convertimos la imagen a base64 para mostrarla en React
        import base64
...
@eel.expose
def api_signer_save(placements: list) -> dict:
    """
    placements = [
        {
            "page": 0,
            "x": 100.0,       # coordenadas en puntos PDF reales
            "y": 200.0,
            "width": 150.0,
            "height": 50.0
        }
    ...
    """
    print(f"[API] >>> Guardando PDF firmado con {len(placements)} firma(s)")
    try:
        if not _signer_session["source_path"]:
            return {"success": False, "error": "No hay PDF cargado."}
        if not _signer_session["signature_path"]:
            return {"success": False, "error": "No hay firma cargada."}
        if not placements:
            return {"success": False, "error": "No hay firmas posicionadas."}
        ...
        result = signer_service.sign(
            _signer_session["source_path"],
            _signer_session["signature_path"],
            placements,
            output_path
        )
...
def api_signer_get_pdf_base64() -> dict:
    """Retorna el PDF actual como base64 para react-pdf."""
    try:
        if not _signer_session["source_path"]:
            return {"success": False, "error": "No hay PDF cargado."}
        import base64
...
def api_signer_get_page_sizes() -> dict:
    """Retorna el tamaño real (en puntos PDF) de cada página."""
    try:
        if not _signer_session["source_path"]:
            return {"success": False, "error": "No hay PDF cargado."}
        ...
        sizes = []
        for page in doc:
            sizes.append({"width": page.rect.width, "height": page.rect.height})
        doc.close()
    ...
    signatures_data = [
        {
            ...
            "placements": [{ "page": 0, "x": 100, "y": 200, "width": 150, "height": 60 }]
        }
    ]
    """
    print(f"[API] >>> Guardando PDF con {len(signatures_data)} firma(s) distintas")
    try:
        if not _signer_session["source_path"]:
            return {"success": False, "error": "No hay PDF cargado."}
        ...
        for sig_data in signatures_data:
            # Convertimos base64 a archivo temporal
            b64 = sig_data["signature_base64"].split(",")[1]
